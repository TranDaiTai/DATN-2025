...
def fetch_titles_list(save_path="titles.json"):
    all_titles_endpoint = "https://emsiservices.com/titles/versions/latest/titles"
    auth = f"Bearer {access_token}"
    headers = {"Authorization": auth}

    response = requests.get(all_titles_endpoint, headers=headers)
    response.raise_for_status()
